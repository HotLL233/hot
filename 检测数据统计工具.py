    ...
    def _write_formatted_excel(self, save_path, results, total_series, column_totals):
        ...
        try:
            with pd.ExcelWriter(save_path, engine="openpyxl") as writer:
                ...
                if not column_totals.empty:
                    total_dict = {"日期": "合计"}
                    for i, col in enumerate(results.columns):
                        total_dict[col] = int(column_totals.iloc[i]) if i < len(column_totals) else 0
                    total_dict["合计"] = int(column_totals.sum())
                    detail_df = pd.concat([detail_df, pd.DataFrame([total_dict])], ignore_index=True)
                # 将 0 值替换为空（不显示），保留日期列
                ...
                grand_total = column_totals.sum() if not column_totals.empty else 0
                total_df.loc[len(total_df)] = ["合计", grand_total]
