...
# -*- coding: utf-8 -*-
"""
检测数据统计工具 v2.2.11 - 双平台同步版
-----------------------------------
新增 (v2.2.10 → v2.2.11)：
1. 新增"导入文件夹"按钮：可选择文件夹，自动递归扫描所有子文件夹内的 .xlsx 文件
"""

# Version: 2.2.11
# 更新说明：
# - 新增 import_folder() API，调用 tkinter askdirectory，使用 os.walk 递归扫描
# - 前端新增"导入文件夹"按钮，调用 pywebview.api.import_folder()
...
# 留空则直连 GitHub
UPDATE_MIRROR_BASE: str = "https://gitee.com/hotll233/hot/releases/download"
...
<body class="light">
  ...
  <aside class="sidebar">
    ...
    <button class="galaxy-btn primary" onclick="importFolder()" style="margin-top:6px;" title="选择文件夹，自动递归扫描所有 .xlsx">
  ...
  <div class="modal-overlay" id="about-modal">
    <div class="modal-box" style="max-width:420px;">
      ...
      <div class="about-info">
        ...
        <div id="update-status" style="font-size:13px;color:var(--text-dim);min-height:20px;"></div>
      </div>
      <div style="text-align:center;margin-top:14px;">
        <button class="galaxy-btn info" id="check-update-btn" onclick="checkUpdateInAbout()" style="width:auto;display:inline-flex;">
  ...
  <script>
    ...
    async function importFolder() {
      ...
      try {
        ...
        if (result.imported_count > 0) {
          showToast('文件夹扫描完成，新导入 ' + result.imported_count + ' 个文件', 'success');
    ...
    // ===================== 在线更新（关于弹窗内） =====================
    async function checkUpdateInAbout() {
      var status = document.getElementById('update-status');
      var btn = document.getElementById('check-update-btn');
      status.innerHTML = '<span style="color:#8b949e;">⏳ 正在检查...</span>';
      btn.disabled = true;
      setStatus('检查更新中...', 'busy');

      try {
        var result = await pywebview.api.check_update();
        if (result.error) {
        ...
        if (!result.has_update) {
          status.innerHTML = '<span style="color:#38ef7d;">✅ 已是最新版本 v' + result.current_version + '</span>';
          setStatus('已是最新版本');
          btn.disabled = false;
          return;
        }
        ...
        // 有新版本
        var notes = result.release_notes || '';
        if (notes.length > 300) notes = notes.substring(0, 300) + '...';
        status.innerHTML = 
          '<span style="color:#f2c94c;">🔔 发现新版本 <b>v' + result.latest_version + '</b></span>' +
        ...
        status.innerHTML = '<span style="color:#f45c43;">网络错误: ' + e + '</span>';
        setStatus('检查更新失败', 'error');
        btn.disabled = false;
      }
    }

    async function doUpdateInline() {
      var status = document.getElementById('update-status');
      status.innerHTML = '<span style="color:#8b949e;">📥 正在下载安装包...</span>';
      ...
      try {
        ...
        status.innerHTML = '<span style="color:#38ef7d;">✅ 安装包已下载，程序将自动重启更新</span>';
        setStatus('更新完成');
      } catch (e) {
        status.innerHTML = '<span style="color:#f45c43;">下载失败: ' + e + '</span>';
        setStatus('下载失败', 'error');
      }
    }
...
class BackendApi:
    ...
    def import_files(self) -> dict:
        ...
        duplicate_count = len(paths) - len(new_paths)
        ...
        if not new_paths:
            ...
            return {
                ...
                "status": f"所选文件均已导入，当前共 {len(self.file_paths)} 个文件",
            }
        ...
        logger.info(
            "追加导入 %d 个文件（已有 %d 个，共计 %d 个）: %s",
            len(new_paths), old_count, len(self.file_paths),
            [os.path.basename(p) for p in new_paths],
        )
        ...
        msg_suffix = f"（新增 {total_new} 个）" if old_count > 0 else ""
        return {
            "files": file_names,
            "results": self._df_to_table_json(
                self._merged_results,
            ...
            "status": f"已导入 {len(self.file_paths)} 个文件{msg_suffix}，统计完成",
        }

    def import_folder(self) -> dict:
        """打开文件夹选择对话框，递归扫描所有子文件夹内的 .xlsx 并导入"""
        tk, filedialog = _lazy_import_tkinter()
        ...
        folder = filedialog.askdirectory(
            title="请选择数据文件夹（将自动扫描子文件夹内所有 .xlsx）",
        )
        root.destroy()
        ...
        for root_dir, _dirs, files in os.walk(folder):
            for fname in files:
                if fname.lower().endswith(".xlsx") and not fname.startswith("~$"):
                    found_paths.append(os.path.join(root_dir, fname))

        logger.info("文件夹扫描：%s，找到 %d 个 .xlsx 文件", folder, len(found_paths))

        if not found_paths:
            return {
                "error": f"文件夹内未找到 .xlsx 文件（已递归扫描子文件夹）",
        ...
        # 去重：过滤掉已导入的文件
        existing_set = set(self.file_paths)
        new_paths = [p for p in found_paths if p not in existing_set]
        duplicate_count = len(found_paths) - len(new_paths)
        if duplicate_count > 0:
            logger.info("跳过 %d 个已导入的重复文件", duplicate_count)
        ...
        if not new_paths:
            ...
            return {
                ...
                "status": f"文件夹内 {len(found_paths)} 个文件均已导入，当前共 {len(self.file_paths)} 个文件",
                "imported_count": 0,
            }
        ...
        Args:
            ...
            rebuild_all: True=全量重建（清空缓存重来），False=增量合并（只处理新文件）
        """
        if not self.file_paths:
            return {"error": "请先导入文件"}

        # ------ 确定要处理的文件列表 ------
        if rebuild_all or not self.file_stats:
            files_to_process = list(self.file_paths)
            sheet_counts: Dict[str, pd.Series] = {}
            self.file_stats.clear()
        else:
            # 增量模式：只处理尚未统计的文件
            files_to_process = [p for p in self.file_paths if p not in self.file_stats]
            # 从已有统计重建 sheet_counts
            sheet_counts = self._rebuild_sheet_counts_from_stats()

        if not files_to_process:
            logger.info("没有新文件需要处理")
            return self._finalize_results(sheet_counts)

        logger.info(
            "处理 %d 个文件（%s模式）",
            len(files_to_process),
            "全量" if rebuild_all else "增量",
        )
    ...
    def _rebuild_sheet_counts_from_stats(self) -> Dict[str, 'pd.Series']:
        """从已有 file_stats 重建跨文件聚合的 sheet_counts"""
        sheet_counts: Dict[str, 'pd.Series'] = {}
        for series_dict in self.file_stats.values():
            for sheet_name, daily_counts in series_dict.items():
                if sheet_name in sheet_counts:
                    ...
                    sheet_counts[sheet_name] = daily_counts.copy()
        return sheet_counts

    def _finalize_results(self, sheet_counts: Dict[str, 'pd.Series']) -> dict:
        """将 sheet_counts 汇总为最终 merged_results"""
    ...
    def download_update(self, download_url: str, mirror_url: str = "") -> dict:
        ...
        try:
            ...
            logger.info("已启动静默安装: %s", installer_path)
            return {"message": "安装包下载完成，正在静默安装..."}
        except Exception as e:
            logger.error("下载更新失败: %s", e)
            return {"error": f"下载更新失败: {e}"}

    @staticmethod
    def _version_greater(v1: str, v2: str) -> bool:
        """比较版本号 v1 > v2（如 "2.2.6" > "2.2.5"）"""
        try:
...
def main(debug: bool = False) -> None:
    ...
    def _on_progress(current: int, total: int, message: str) -> None:
        ...
        # 用 json.dumps 转义文件名中的特殊字符（引号/反斜杠等）
        safe_msg = json.dumps(f"{message} ({current}/{total})", ensure_ascii=False)
        window.evaluate_js(
            f"updateProgress({percent}, '处理中…', {safe_msg})"
        )
