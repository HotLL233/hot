...
# -*- coding: utf-8 -*-
"""
检测数据统计工具 v2.2.3 - 多批次导入版
-----------------------------------
新增 (v2.2.2 → v2.2.3)：
1. 导入文件改为追加模式：多次导入不同文件夹的文件不会清除之前的导入
2. 重复文件自动去重：已有文件不会重新统计
3. _process_files 新增 rebuild_all 参数支持增量合并
"""

# Version: 2.2.3
# 更新说明：
# - 导入文件由替换改为追加模式，支持从不同路径分批导入
# - 同名文件自动去重，仅处理新增文件
# - _process_files 增量合并策略：已处理文件跳过，新文件数据累加到已有统计
