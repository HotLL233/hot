...
# -*- coding: utf-8 -*-
"""
检测数据统计工具 v2.2.7 - 稳定性修复版
-----------------------------------
修复 (v2.2.6 → v2.2.7)：
1. 导出文件删除：补 FileNotFoundError 异常处理，避免并发冲突崩溃
2. 表格显示：NaN 与真 0 区分，无数据显示空白而非 0
3. 进度回调：json.dumps 转义文件名，防止引号导致 JS 语法错误
"""

# Version: 2.2.7
# 更新说明：
# - _write_formatted_excel: os.remove() 补 FileNotFoundError + 内部 alt_path 一并处理
# - _df_to_table_json: fillna(0) → 逐行 pd.isna()，NaN 传空串，真 0 保留
# - _df_to_table_json: 补 _lazy_import_pandas() 调用（修复上一版回归）
# - main() 进度回调: 字符串拼接 → json.dumps() 转义特殊字符
