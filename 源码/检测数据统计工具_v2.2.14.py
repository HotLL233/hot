...
if getattr(sys, 'frozen', False) and os.name == "nt":
    # 打包后：%LOCALAPPDATA%\检测数据统计工具\logs
    ...
    async function importFolder() {
      ...
      try {
        ...
        if (result.imported_count > 0) {
          showToast('导入完成：递归扫描新增 ' + result.imported_count + ' 个文件，共 ' + (result.files || []).length + ' 个', 'success');
...
class BackendApi:
    ...
    def import_folder(self) -> dict:
        ...
        root.withdraw()
        root.attributes("-topmost", True)
        folder = filedialog.askdirectory(title="请选择父级文件夹（将递归扫描所有子文件夹内的 .xlsx）")
        root.destroy()
        ...
        for root_dir, _dirs, files in os.walk(folder):
            ...
            for fname in files:
                if fname.lower().endswith(".xlsx") and not fname.startswith("~$"):
                    sub_found.append(os.path.join(root_dir, fname))
            if sub_found:
                all_found_paths.extend(sub_found)
                folder_count += 1

        if not all_found_paths:
            logger.info("文件夹 %s 内未找到 .xlsx", folder)
            file_names = [os.path.basename(p) for p in self.file_paths]
            return {
                "files": file_names,
                "results": self._df_to_table_json(
                ...
                "status": "「%s」内未找到有效文件" % os.path.basename(folder),
                "imported_count": 0,
            }

        logger.info(
            "递归扫描 %s：%d 个子文件夹，共 %d 个 .xlsx",
            folder, folder_count, len(all_found_paths),
        )
        ...
        if not new_paths:
            ...
            return {
                ...
                "status": "「%s」内 %d 个文件均已导入，当前共 %d 个" % (
                    os.path.basename(folder), len(all_found_paths), len(self.file_paths)),
                "imported_count": 0,
            }
        ...
        return {
            ...
            "status": "从「%s」递归导入 %d 个新文件（共 %d 个）" % (
                os.path.basename(folder), len(new_paths), len(self.file_paths)),
