...
# -*- coding: utf-8 -*-
"""
检测数据统计工具 v2.2.0 - 性能优化版
-----------------------------------
优化点：
1. 启动速度优化：延迟加载 pandas 等重型库
2. 日志文件位置：改为程序所在目录下的 logs 文件夹
3. 导出目录选择：首次导出时让用户选择导出目录，并记住选择
4. 性能优化：减少不必要的 pandas 操作，使用更高效的日期解析
5. 内存优化：使用生成器处理大文件
"""

# Version: 2.2.0
# 更新说明：
# - 启动时延迟导入 pandas、openpyxl 等重型库
# - 日志文件改为程序目录下的 logs 文件夹
# - 首次导出时让用户选择导出目录，并记住选择
# - 优化日期解析性能，减少 pandas 操作
# - 使用生成器分批处理大文件
# - 添加进度显示和取消功能
...
def read_sheet_chunks(file_path: str, sheet_name: str, chunk_size: int = 1000) -> Generator[pd.DataFrame, None, None]:
    """分块读取工作表，减少内存占用"""
    pd, _ = _lazy_import_pandas()
        ...
        for file_idx, file_path in enumerate(self.file_paths):
            ...
            for sheet_idx, sheet_name in enumerate(xl.sheet_names):
                ...
                try:
                    # 使用生成器分块读取
                    all_chunks = []
                    for chunk in read_sheet_chunks(file_path, sheet_name):
