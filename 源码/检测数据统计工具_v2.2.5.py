...
# -*- coding: utf-8 -*-
"""
检测数据统计工具 v2.2.5 - 自适应列版
-----------------------------------
新增 (v2.2.4 → v2.2.5)：
1. 自适应列识别：通过关键词匹配自动定位日期列和批号列
2. 数据行检测改为全列扫描（前50行 × 前30列），不再限定A列
3. 扩展关键词库：覆盖实验室常见表头（报告日期/分析日期/样品批号等）
"""

# Version: 2.2.5
# 更新说明：
# - _detect_columns 自动识别日期列和批号列（关键词匹配表头）
# - _detect_data_start_row 改为全列扫描，日期在任意列均可定位
# - 日期关键词：日期/date/时间/time/检测日期/采样日期/报告日期/分析日期/送检日期/检验日期
# - 批号关键词：批号/batch/lot/编号/序号/样品批号/样品编号/样品号/样品名称/实验编号/样本号/code/id
