...
# -*- coding: utf-8 -*-
"""
检测数据统计工具 v2.2.8 - 下载加速版
-----------------------------------
优化 (v2.2.7 → v2.2.8)：
1. 下载缓冲区 8KB → 64KB，高延迟下提速 ~8x
2. 支持 Gitee 镜像源，国内优先下载，失败自动回退 GitHub
3. 下载超时策略优化，避免慢速网络假死
"""

# Version: 2.2.8
# 更新说明：
# - download_update: 读取缓冲区 8192 → 65536，减少跨洋往返开销
# - download_update: 优先尝试镜像 URL，失败自动切换 GitHub 源
# - 新增 UPDATE_MIRROR_BASE 配置项，指向 Gitee Releases 地址
# - 连接超时 15s，读取不限时，避免大文件下载中途断开
