...
# Version: 2.0.5
# 更新说明：
# - JSON 序列化改用向量化处理，导入速度大幅提升
# - 后台预加载 pandas，消除首次导入延迟
...
class BackendApi:
    ...
    def _df_to_table_json(self, results: pd.DataFrame, total_series: pd.Series,
        ...
        """将 DataFrame 转换为前端表格需要的 JSON 结构（向量化优化）"""
        if results.empty:
            return {"columns": [], "rows": [], "totals": [], "mode": mode}

        columns = ["日期"] + list(results.columns) + ["合计"]
        # 向量化：一次 fillna + astype 替代逐格 notna 检查
        num = results.fillna(0).astype(int)
        rows = []
        for row in num.itertuples():
            date = row.Index
            vals = list(row[1:])
            vals.append(int(total_series.get(date, 0)))
            rows.append([date] + vals)

        totals = ["合计"]
        for col in results.columns:
            totals.append(int(column_totals.get(col, 0)))
        totals.append(int(column_totals.sum()))
...
def main():
    ...
    logger.info("应用启动 v2.0.5")
    threading.Thread(target=_get_pd, daemon=True).start()   # 后台预加载 pandas
