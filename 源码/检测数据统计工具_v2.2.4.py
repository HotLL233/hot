...
# -*- coding: utf-8 -*-
"""
检测数据统计工具 v2.2.4 - 自适应表头版
-----------------------------------
新增 (v2.2.3 → v2.2.4)：
1. 自动识别数据起始行：扫描 A 列前 50 行找到第一个有效日期
2. 不再固定 skiprows=2，兼容不同表头行数的模板
3. 未检测到日期时回退到默认 2 行（向后兼容）
"""

# Version: 2.2.4
# 更新说明：
# - read_sheet_data 自动扫描 A 列，以第一个有效日期为数据起点
# - 新增 _detect_data_start_row 函数，支持 YYYYMMDD / YYYY-MM-DD 格式识别
# - 不同模板（0 行、2 行、5 行表头）的 Excel 现在可混用导入
